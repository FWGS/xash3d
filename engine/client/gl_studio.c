...
static qboolean R_StudioComputeBBox( vec3_t bbox[8] )
{
	...
	if( !bbox && R_CullModel( e, origin, studio_mins, studio_maxs, studio_radius ))
...
void R_StudioLerpMovement( cl_entity_t *e, double time, vec3_t origin, vec3_t angles )
{
	...
	{
		InterpolateAngles(e->latched.prevangles, e->curstate.angles, angles, f);
	}
...
void R_StudioCalcRotations( cl_entity_t *e, float pos[][3], vec4_t *q, mstudioseqdesc_t *pseqdesc, mstudioanim_t *panim, float f )
{
	...
	{
		Mod_StudioCalcBoneQuaternion( frame, s, pbone, panim, adj, q[i] );
		Mod_StudioCalcBonePosition( frame, s, pbone, panim, adj, pos[i] );
	}
...
void R_StudioMergeBones( cl_entity_t *e, model_t *m_pSubModel )
{
	...
	panim = Mod_StudioGetAnim( m_pStudioHeader, m_pSubModel, pseqdesc );
...
void R_StudioSetupBones( cl_entity_t *e )
{
	...
	panim = Mod_StudioGetAnim( m_pStudioHeader, RI.currentmodel, pseqdesc );
	R_StudioCalcRotations( e, pos, q, pseqdesc, panim, f );
	...
	if( pseqdesc->numblends > 1 )
	{
		...
		Mod_StudioSlerpBones( m_pStudioHeader->numbones, q, pos, q2, pos2, s );
		...
		if( pseqdesc->numblends == 4 )
		{
			...
			s = (e->curstate.blending[0] * dadt + e->latched.prevblending[0] * (1.0f - dadt)) / 255.0f;
			Mod_StudioSlerpBones( m_pStudioHeader->numbones, q3, pos3, q4, pos4, s );

			s = (e->curstate.blending[1] * dadt + e->latched.prevblending[1] * (1.0f - dadt)) / 255.0f;
			Mod_StudioSlerpBones( m_pStudioHeader->numbones, q, pos, q3, pos3, s );
		}
	}
	...
	if( g_studio.interpolate && e->latched.sequencetime && ( e->latched.sequencetime + 0.2f > g_studio.time ) && ( e->latched.prevsequence < m_pStudioHeader->numseq ))
	{
		...
		if( pseqdesc->numblends > 1 )
		{
			...
			Mod_StudioSlerpBones( m_pStudioHeader->numbones, q1b, pos1b, q2, pos2, s );
			...
			if( pseqdesc->numblends == 4 )
			{
				...
				Mod_StudioSlerpBones( m_pStudioHeader->numbones, q1b, pos1b, q3, pos3, s );
			}
		}

		s = 1.0f - ( g_studio.time - e->latched.sequencetime ) / 0.2f;
		Mod_StudioSlerpBones( m_pStudioHeader->numbones, q, pos, q1b, pos1b, s );
	}
	else
	{
...
void R_StudioBuildNormalTable( void )
{
	...
	if( e->curstate.rendercolor.r || e->curstate.rendercolor.g || e->curstate.rendercolor.b )
		pglColor4ub( e->curstate.rendercolor.r, e->curstate.rendercolor.g, e->curstate.rendercolor.b, 255 );
	else pglColor4ub( 255, 255, 255, 255 );
}
...
static void R_StudioCalcAttachments( void )
{
	...
	for( i = 0; i < min( MAXSTUDIOATTACHMENTS, m_pStudioHeader->numattachments ); i++ )
	{
...
void R_StudioDynamicLight( cl_entity_t *ent, alight_t *plight )
{
	...
	total = max( max( finalLight[0], finalLight[1] ), finalLight[2] );
...
float *R_LightLambert( vec4_t light[MAX_LOCALLIGHTS], vec3_t normal, vec3_t color, float alpha )
{
	...
	for( i = 0; i < g_studio.numlocallights; i++ )
	{
		...
		if( r > 0.0f )
		{
			...
			localLight[0] = min( g_studio.locallightcolor[i].r * r * light[i][3], 255.0f );
			localLight[1] = min( g_studio.locallightcolor[i].g * r * light[i][3], 255.0f );
			localLight[2] = min( g_studio.locallightcolor[i].b * r * light[i][3], 255.0f );
			VectorScale( localLight, ( 1.0f / 255.0f ), localLight );

			finalLight[0] = min( finalLight[0] + localLight[0], 1.0f );
			finalLight[1] = min( finalLight[1] + localLight[1], 1.0f );
			finalLight[2] = min( finalLight[2] + localLight[2], 1.0f );
		}
	}
...
===============
*/
_inline void R_StudioDrawNormalMesh( short *ptricmds, vec3_t *pstudionorms, float s, float t, float alpha )
{
	float	*lv;
	int	i;
...
===============
*/
_inline void R_StudioDrawFloatMesh( short *ptricmds, vec3_t *pstudionorms, float alpha )
{
	float	*lv;
	int	i;
...
===============
*/
_inline void R_StudioDrawChromeMesh( short *ptricmds, vec3_t *pstudionorms, float s, float t, float scale, float alpha )
{
	float	*lv, *av;
	int	i, idx;
...
static void R_StudioDrawPoints( void )
{
	...
	if( RI.currententity->curstate.renderfx == kRenderFxGlowShell )
	{
		...
		shellscale = max( factor, RI.currententity->curstate.renderamt * factor );
	...
	for( j = k = 0; j < m_pSubModel->nummesh; j++ )
	{
		...
		if( RI.currententity->curstate.rendermode == kRenderTransAdd )
		{
			for( i = 0; i < pmesh[j].numnorms; i++, k++, pstudionorms++, pnormbone++ )
			{
				...
				VectorSet( g_studio.lightvalues[k], alpha, alpha, alpha );
			}
		}
		else
		{
...
static void GL_StudioDrawShadow( void )
{
	...
	{
		float	color = 1.0 - (r_shadowalpha.value * 0.5);
...
void R_RunViewmodelEvents( void )
{
	...
	for( i = 0; i < 4; i++ )
		VectorCopy( cl.predicted.origin, RI.currententity->attachment[i] );
...
void R_DrawViewModel( void )
{
	...
	alpha = R_ComputeFxBlend( view ) / 255.0f;
	if( !R_ModelOpaque( view->curstate.rendermode ) && alpha <= 0.0f )
		return; // invisible ?
