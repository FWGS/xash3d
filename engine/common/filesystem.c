...
TODO: Move to different file after making it more convenient. (Timestamp check, temporary file, ...)

Used buffer for one-time write but current console buffer limited it so it was removed.

=============================================================================
*/

void Con_ListMaps( void )
{
	if( Cmd_Argc() != 2 )
	{
	...
	char mapsLoc[ 256 ], mapName[ 256 ], seperator[ 64 ];
	char *argStr = Cmd_Argv( 1 ); //Substr
	unsigned short listIndex = 0, mapCount = 0;

	stringlistinit( &mapList );
	Q_snprintf( mapsLoc, 255, "./%s/maps/", GI->basedir );
	listdirectory( &mapList, mapsLoc, false );
	stringlistsort( &mapList );

	Q_strcpy( seperator, "---------------------" );
	Msg( "%s\n", seperator );

	if( Q_strcmp( argStr, "*" ) != 0 )
	{
		unsigned short follower = 0;
		for( ; listIndex != mapList.numstrings; ++listIndex )
		{
			Q_strncpy( mapName, mapList.strings[ listIndex ] , sizeof( mapName ) - 1 );
			mapName[ 255 ] = '\0';
			if ( Q_stristr( mapName, ".bsp" ) == NULL )
				continue;
			++mapCount;
			if ( Q_stristr(mapName, argStr) != NULL )
			{
				Msg("%s\n", mapName);
				++follower;
			}
		}
		Msg( "%s\nGamedir: %s - Map count: %d - Maps listed: %d\n", seperator, GI->basedir, mapCount, follower );
	}
	else
	{
		for ( ; listIndex != mapList.numstrings; ++listIndex )
		...
		Msg( "%s\nGamedir: %s - Map count: %d\n", seperator, GI->basedir, mapCount, mapCount );
	}
