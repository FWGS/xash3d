...
void II_step_two(struct StaticData * psd, unsigned int *bit_alloc,float fraction[2][4][SBLIMIT],int *scale,struct frame *fr,int x1)
{
    ...
    for (i=0;i<jsbound;i++,alloc1+=(1<<step))
    {
      ...
      for (j=0;j<stereo;j++)
      {
        if ( (ba=*bita++) )
        {
          ...
          if( (d1=alloc2->d) < 0)
          {
            float cm=psd->muls[k][scale[x1]];
            fraction[j][0][i] = ((float) ((int)getbits(psd, k) + d1)) * cm;
            fraction[j][1][i] = ((float) ((int)getbits(psd, k) + d1)) * cm;
            fraction[j][2][i] = ((float) ((int)getbits(psd, k) + d1)) * cm;
          }
          else
          {
            int *table[10];
    ...
    for (i=jsbound;i<sblimit;i++,alloc1+=(1<<step))
    {
      ...
      if ( (ba=*bita++) )
      {
        ...
        if( (d1=alloc2->d) < 0)
        {
          ...
          fraction[1][0][i] = (fraction[0][0][i] = (float) ((int)getbits(psd, k) + d1) ) * cm;
          fraction[1][1][i] = (fraction[0][1][i] = (float) ((int)getbits(psd, k) + d1) ) * cm;
          fraction[1][2][i] = (fraction[0][2][i] = (float) ((int)getbits(psd, k) + d1) ) * cm;
